...
LEGACY_FIXTURE_MODEL_ID = "fixture"
...
class FixtureChordOCR:
    ...
    def __init__(self, fixtures_dir: Path, *, model_id: str = DEFAULT_MODEL) -> None:
    ...
    def extract(self, request: ChordExtractionRequest) -> Sequence[ChordDetection]:
        ...
        for model_id in _fixture_lookup_model_ids(self.model_id):
            key = fixture_key(
                request.image_path,
        ...
        raise FixtureNotFoundError(
            f"No chord-OCR fixture for image {request.image_path} (keys={attempted_keys})"
        )
...
def read_fixture(path: Path) -> list[ChordDetection]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    return _load_detections(payload["detections"])


def _load_detections(raw: Iterable[dict]) -> list[ChordDetection]:
    return [
        ChordDetection(
...
def _fixture_lookup_model_ids(model_id: str) -> tuple[str, ...]:
    if model_id == LEGACY_FIXTURE_MODEL_ID:
        return (model_id,)
    return (model_id, LEGACY_FIXTURE_MODEL_ID)
