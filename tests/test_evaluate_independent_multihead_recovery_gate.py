...
from scripts.experiments import evaluate_independent_multihead_recovery_gate as evaluator
from scripts.experiments import freeze_third_score_heldout as freezer
from scripts.experiments import run_independent_multihead_recovery_gate as multihead_runner
...
def paired_rows_with_canonical_pitch_difference() -> list[dict[str, Any]]:
    ...
    baseline["canonical_prediction_sha256"] = multihead_runner.inference._hash_json(
...
def test_evaluates_paired_lanes_with_nontrivial_mapping_and_ignores_accidentals(
    ...
    result = evaluator.evaluate_independent_multihead_recovery_gate(
        fixture["sealed"],
...
def test_frozen_hash_drift_fails_before_opening_musicxml(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="snapshot hash drift"):
        evaluator.evaluate_independent_multihead_recovery_gate(
            fixture["sealed"],
...
def test_help_names_expected_a_medio_palo_transcription_path(
...
def _frozen_fixture(tmp_path: Path) -> dict[str, Any]:
    ...
    selection.write_text('{"selected":"system_007"}\n', encoding="utf-8")
    ...
    freezer._write_json(
        prepared_path,
        {
            ...
            "independent_multihead_recovery_gate": {
                "config_id": multihead_runner.recovery.EDGE_SAFE_STEM_MULTIHEAD_CONFIG_ID,
                "parameters": dict(multihead_runner.recovery.EDGE_SAFE_STEM_MULTIHEAD_PARAMETERS),
    ...
    baseline_inference = namespace_root / multihead_runner.DEFAULT_INFERENCE_DIRNAME
    ...
    pair_dir = baseline_inference / multihead_runner.PAIR_DIRNAME
    pair_dir.mkdir()
    ...
    freezer._write_json(
        pair_manifest,
        {
            "schema_version": multihead_runner.SCHEMA_VERSION,
            "kind": "independent_multihead_recovery_paired_prediction_manifest",
            "version": multihead_runner.PAIR_VERSION,
            "target": {
                "slug": evaluator.TARGET_SLUG,
                "system_index": evaluator.TARGET_SYSTEM_INDEX,
            },
    ...
    paired_frozen = namespace_root / "multihead_recovery_frozen"
    paired_frozen.mkdir()
    groups = {
        ...
        "implementations": [Path(evaluator.__file__), Path(multihead_runner.__file__)],
    }
    pins = {
        role: [
...
def _baseline_lane(notes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "canonical_prediction_sha256": multihead_runner.inference._hash_json(canonical),
