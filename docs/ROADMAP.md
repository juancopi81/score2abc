...
Next main M2 focus: transcribe and evaluate the frozen Desde Lejos system-7 dotted-hollow
gate without rewriting either prediction lane. The evaluator must verify head pixels,
augmentation dots, and onset grouping rather than relying on MusicXML pitch/count agreement alone. Keep
multi-head recovery and visual-key state opt-in, and keep multi-staff grouping as a separate
segmentation slice. Do not integrate internal double-bar key changes, local staff tracking,
the older Chispazo hollow-head rule, or meter-based note deletion yet. Default pipeline
integration remains gated on independent exact-event/rest evidence.
