...
Next main M2 focus: acquire a genuinely unseen polyphonic score or system and freeze
the selected multi-head rule before transcription. The remaining untouched scores in
the current corpus are monophonic, so they cannot exercise positive chord recovery.
Keep both recovery rules outside runtime, the visual-key path opt-in, and multi-staff
grouping as a separate segmentation slice. Do not integrate internal double-bar key
changes, local staff tracking, the Chispazo hollow-head rule, or meter-based note
deletion yet. Default pipeline integration remains gated on independent exact-event/rest
evidence.
