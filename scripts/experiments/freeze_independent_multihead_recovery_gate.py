"""Prepare the truth-blind A medio palo multihead-recovery gate.

The fixed target is local-restricted A medio palo system 7. Preparation may
inspect only pipeline images and metadata. It seals seven automatic crops and
...
EVALUATOR_VERSION = "independent-multihead-recovery-gate-v1"
A_MEDIO_PALO_SLUG = "jaime-llanos_7_a-medio-palo_pasillo_m-garavito-w"
TARGET_SYSTEM_INDEX = 7
EXPECTED_CROP_COUNT = 7

INDEPENDENT_MULTIHEAD_RECOVERY_GATE = base.HeldoutGateSpec(
    key="independent_multihead_recovery",
...
DEFAULT_CANDIDATE_POOL = (
    base.Candidate(A_MEDIO_PALO_SLUG, TARGET_SYSTEM_INDEX, "fixed_preregistered_target"),
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = prepare_independent_multihead_recovery_gate(
            args.out_dir,
...
def prepare_independent_multihead_recovery_gate(
    out_dir: Path,
    *,
    ...
    """Seal the fixed seven-crop target without opening target truth."""
    _validate_target(candidate_pool)
    metadata_path = out_dir / A_MEDIO_PALO_SLUG / "metadata.json"
    if not metadata_path.is_file():
        raise ValueError(f"Missing pipeline metadata for multihead gate: {metadata_path}")
    ...
    result = base.prepare_heldout_score(
        ...
        gate=INDEPENDENT_MULTIHEAD_RECOVERY_GATE,
    )
    prepared_path = Path(result["prepared_manifest"])
    prepared = base._read_json(prepared_path)
    expected_target = {"slug": A_MEDIO_PALO_SLUG, "system_index": TARGET_SYSTEM_INDEX}
    if prepared.get("target") != expected_target:
        raise ValueError(f"Unexpected independent multihead target: {prepared.get('target')}")
    crops = prepared["artifacts"]["crops"]
    if len(crops) != EXPECTED_CROP_COUNT:
        raise ValueError(
            f"Independent multihead gate requires {EXPECTED_CROP_COUNT} crops, got {len(crops)}"
        )

    context_record = _write_context(prepared_path.parent, metadata_path=metadata_path)
    prepared["independent_multihead_recovery_gate"] = {
        ...
        "recovery_contract": (
            "add at most two companions to each existing x group; never delete or reposition "
            "baseline candidates"
        ),
    ...
    prepared["forbidden_truth_paths"].extend(
        [
            f"out/local_restricted/{A_MEDIO_PALO_SLUG}/**/*truth*",
            f"out/local_restricted/{A_MEDIO_PALO_SLUG}/**/*.musicxml",
            f"out/local_restricted/{A_MEDIO_PALO_SLUG}/**/*.mxl",
        ]
    )
...
def _validate_target(candidate_pool: Sequence[base.Candidate]) -> None:
    ...
    if tuple(candidate_pool) != expected:
        raise ValueError("Independent multihead gate target is fixed and cannot be substituted")
...
def _write_context(namespace_root: Path, *, metadata_path: Path) -> dict[str, str]:
    ...
    payload = {
        "schema_version": 1,
        "kind": "independent_multihead_recovery_allowed_context",
        "truth_accessed": False,
        "truth_used": False,
        "allowed_context": {
            "clef": "treble",
    ...
    *,
    ...
    payload = {
        ...
        "required_checks": [
            ...
            "verify recovery adds at most two companions per existing x group",
