"""Run and seal the independent A medio palo multihead-recovery gate.
...
from scripts.experiments import freeze_independent_multihead_recovery_gate as gate  # noqa: E402
...
SCHEMA_VERSION = 2
PAIR_VERSION = "independent-multihead-recovery-paired-predictions-v1"
DEFAULT_INFERENCE_DIRNAME = "baseline_inference_v1"
PAIR_DIRNAME = "multihead_recovery_v1"
    ...
    *,
    ...
    if pair_dir.exists() or frozen_dir.exists():
        raise FileExistsError(
            "Refusing multihead baseline resume after paired or frozen output already exists"
        )
    manifest_path = inference_dir / "manifest.json"
    manifest = inference._read_json(manifest_path)
    expected_gate = inference.GATE_CONFIGS[gate.INDEPENDENT_MULTIHEAD_RECOVERY_GATE.prepare_kind]
    prepared = base._read_json(prepared_manifest_path)
    if (
        ...
        raise ValueError("Existing multihead baseline inference is not resumable")
    validated = inference._verify_inference_binding(
        prepared_manifest_path,
        model_dir=model_dir,
        inference_dir=inference_dir,
    ...
    if expected_count != gate.EXPECTED_CROP_COUNT:
        raise ValueError("Existing multihead baseline inference count contract drift")
    return {
...
def materialize_paired_recovery(
    ...
    *,
    ...
    if output_dir.exists():
        raise FileExistsError(f"Refusing to overwrite create-once multihead recovery: {output_dir}")
    if len(rows) != gate.EXPECTED_CROP_COUNT:
        raise ValueError(
            f"Independent multihead gate requires {gate.EXPECTED_CROP_COUNT} inference rows, "
            f"got {len(rows)}"
        )
    selector = recovery.selector_config_from_model(model_payload)
    temp_dir = output_dir.with_name(f".{output_dir.name}.tmp")
    if temp_dir.exists():
        raise FileExistsError(f"Refusing stale multihead-recovery temporary output: {temp_dir}")
    ...
    try:
        ...
        inference._write_json(
            parameters_path,
            {
                ...
                "kind": "independent_multihead_recovery_fixed_parameters",
        ...
        manifest = {
            "schema_version": SCHEMA_VERSION,
            "kind": "independent_multihead_recovery_paired_prediction_manifest",
            ...
            "recovery_contract": (
                "add at most two companions per existing x group; never delete or reposition "
                "baseline candidates"
            ),
            ...
            "recovered_head_count": sum(
                int(row["lanes"]["multihead_recovery"]["recovered_head_count"]) for row in pair_rows
            ),
...
def _validate_row_context(row: Mapping[str, Any], *, expected_target: Mapping[str, Any]) -> None:
    if row.get("truth_used") is not False:
        raise ValueError("Independent multihead inference row is not truth-blind")
    identity = row.get("identity")
    if not isinstance(identity, Mapping):
        raise ValueError("Independent multihead inference row has no identity")
    if str(identity.get("slug")) != str(expected_target["slug"]) or int(
        identity.get("system_index", -1)
    ) != int(expected_target["system_index"]):
        raise ValueError("Independent multihead inference target substitution")
    ...
    if context != expected:
        raise ValueError(
            "Independent multihead gate requires unknown key and unsupported meter context"
        )
    if row.get("decoder_status") != "not_applied_missing_expected_measure_beats":
        raise ValueError("Independent multihead gate must not run rhythm/rest decoding")
...
def _verify_additive_recovery(
    ...
    *,
    ...
    group_counts = {group: group_indices.count(group) for group in set(group_indices)}
    max_per_group = int(
        recovery.EDGE_SAFE_STEM_MULTIHEAD_PARAMETERS["maximum_recovered_heads_per_group"]
    )
    if any(count > max_per_group for count in group_counts.values()):
        raise ValueError("Recovery exceeded the per-group companion cap")
    ...
    if len(recovered) > before * max_per_group:
        raise ValueError("Recovery exceeded the total companion cap")
...
def verify_paired_contract(
    ...
    *,
    ...
    if len(paired_rows) != len(generic_rows) or len(paired_rows) != gate.EXPECTED_CROP_COUNT:
        raise ValueError("Paired/generic multihead row count mismatch")
    recovered_total = 0
    for paired, generic in zip(paired_rows, generic_rows, strict=True):
        ...
        if paired.get("identity") != generic.get("identity"):
            raise ValueError("Paired/generic multihead identity mismatch")
        lanes = paired.get("lanes")
        if not isinstance(lanes, Mapping):
            raise ValueError("Paired multihead row has no lanes")
        baseline_lane = lanes["baseline_generic"]
        recovery_lane = lanes["multihead_recovery"]
        ...
        if any(count > max_per_group for count in recovered_group_counts.values()):
            raise ValueError("Recovery exceeded the per-group companion cap")
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "kind": "independent_multihead_recovery_additive_invariance",
        "passed": True,
        "truth_accessed": False,
        "truth_used": False,
        ...
        "maximum_two_companions_per_existing_x_group": True,
...
def freeze_paired_recovery(
    ...
    *,
    ...
    """Create an independent immutable snapshot of the paired multihead gate."""
    namespace_root = prepared_manifest_path.parent
    prepared = base._read_json(prepared_manifest_path)
    _validate_prepared_gate(prepared_manifest_path, prepared)
    generic_manifest = inference._read_json(inference_dir / "manifest.json")
    ...
    if output_dir.exists():
        raise FileExistsError(f"Paired multihead freeze already exists: {output_dir}")
    if temp_dir.exists():
        raise FileExistsError(f"Stale paired multihead freeze exists: {temp_dir}")
    ...
    try:
        ...
        freeze_payload = {
            "schema_version": SCHEMA_VERSION,
            "kind": "independent_multihead_recovery_paired_freeze",
        ...
        inference._write_json(
            ...
            {
                "schema_version": SCHEMA_VERSION,
                "kind": "independent_multihead_recovery_paired_sealed_manifest",
                "status": "frozen_awaiting_truth",
                "truth_accessed": False,
                "truth_used": False,
                "target": prepared["target"],
...
def verify_paired_freeze(output_dir: Path) -> None:
    ...
    if freeze.get("status") != "frozen_awaiting_truth" or freeze.get("truth_accessed") is not False:
        raise ValueError("Paired multihead freeze is not truth-blind/frozen")
    if sealed.get("status") != "frozen_awaiting_truth" or sealed.get("truth_accessed") is not False:
        raise ValueError("Paired multihead seal is not truth-blind/frozen")
    if inference._sha256(output_dir / "freeze.json") != str(sealed["freeze"]["sha256"]):
        raise ValueError("Paired multihead sealed freeze hash drift")
    if (
        freeze.get("config_id") != recovery.EDGE_SAFE_STEM_MULTIHEAD_CONFIG_ID
        or freeze.get("parameters") != recovery.EDGE_SAFE_STEM_MULTIHEAD_PARAMETERS
    ):
        raise ValueError("Paired multihead fixed parameters drift")
    pins = freeze.get("pins")
    if not isinstance(pins, Mapping):
        raise ValueError("Paired multihead freeze has no provenance pins")
    required_roles = {
        "paired_predictions",
        "paired_artifacts",
        "prepared_and_source",
    ...
    if set(pins) != required_roles:
        raise ValueError("Paired multihead freeze provenance roles drift")
    for role in sorted(pins):
        if not pins[role]:
            raise ValueError(f"Paired multihead freeze provenance role is empty: {role}")
        for pin in pins[role]:
            ...
            if inference._sha256(snapshot) != str(pin["snapshot_sha256"]):
                raise ValueError(f"Paired multihead snapshot hash drift: {snapshot}")
            if pin["source_sha256"] != pin["snapshot_sha256"]:
                raise ValueError(f"Paired multihead source/snapshot mismatch: {snapshot}")
...
def _validate_prepared_gate(path: Path, prepared: Mapping[str, Any]) -> None:
    ...
    if prepared.get("kind") != gate.INDEPENDENT_MULTIHEAD_RECOVERY_GATE.prepare_kind:
        raise ValueError("Prepared manifest is not the independent multihead-recovery gate")
    if prepared.get("target") != expected_target:
        raise ValueError("Independent multihead-recovery target substitution")
    config = prepared.get("independent_multihead_recovery_gate")
    if not isinstance(config, Mapping):
        raise ValueError("Prepared manifest has no multihead-recovery contract")
    if (
        config.get("config_id") != recovery.EDGE_SAFE_STEM_MULTIHEAD_CONFIG_ID
        or config.get("parameters") != recovery.EDGE_SAFE_STEM_MULTIHEAD_PARAMETERS
    ):
        raise ValueError("Prepared multihead-recovery parameters drift")
    if config.get("truth_accessed") is not False or prepared.get("truth_accessed") is not False:
        raise ValueError("Prepared multihead-recovery gate is not truth-blind")
    base._verify_prepared_manifest(
        path.parent,
        path,
        prepared,
        expected_kind=gate.INDEPENDENT_MULTIHEAD_RECOVERY_GATE.prepare_kind,
    )
