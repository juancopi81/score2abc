"""Evaluate the frozen independent multihead-recovery gate exactly once.

The evaluator is deliberately separate from the freezer and runner. It
...
Expected transcription path:
  ...
  a_medio_palo_system_007.musicxml
"""
...
from scripts.experiments import (  # noqa: E402
    freeze_independent_multihead_recovery_gate as multihead_gate,
)
from scripts.experiments import freeze_third_score_heldout as freezer  # noqa: E402
from scripts.experiments import (  # noqa: E402
    run_independent_multihead_recovery_gate as multihead_runner,
)

SCHEMA_VERSION = 1
DEFAULT_EVALUATION_VERSION = "v1"
TARGET_SLUG = "jaime-llanos_7_a-medio-palo_pasillo_m-garavito-w"
TARGET_SYSTEM_INDEX = 7
OUTPUT_SUBDIR = "vlm_melody_independent_multihead_recovery_gate"
DEFAULT_NAMESPACE = "v1"
EXPECTED_TRANSCRIPTION_FILENAME = "a_medio_palo_system_007.musicxml"
...
GATE_SPEC = multihead_gate.INDEPENDENT_MULTIHEAD_RECOVERY_GATE
LANE_BASELINE = "baseline_generic"
LANE_RECOVERED = "multihead_recovery"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("sealed_manifest", type=Path, help="Frozen gate sealed_manifest.json")
    parser.add_argument(
        "--musicxml",
        type=Path,
        required=True,
    ...
    try:
        result = evaluate_independent_multihead_recovery_gate(
...
def evaluate_independent_multihead_recovery_gate(
    ...
    *,
    ...
    frozen = verify_frozen_multihead_gate(sealed_manifest_path)
    ...
    if prior_evaluations:
        raise FileExistsError(
            "Independent multihead evaluation already exists: "
            + ", ".join(str(path) for path in prior_evaluations)
        )
    stale_temps = sorted(namespace_root.glob(".evaluation_*.tmp"))
    if stale_temps:
        raise FileExistsError(
            "Stale independent multihead evaluation directory exists: "
    ...
    try:
        ...
        manifest = {
            ...
            "kind": "independent_multihead_recovery_post_freeze_evaluation_manifest",
        ...
        verified_again = verify_frozen_multihead_gate(sealed_manifest_path)
        if (
            verified_again["freeze_sha256"] != frozen["freeze_sha256"]
            or verified_again["sealed_sha256"] != frozen["sealed_sha256"]
        ):
            raise ValueError("Frozen independent multihead gate changed during evaluation")
...
def verify_frozen_multihead_gate(sealed_manifest_path: Path) -> dict[str, Any]:
    ...
    if sealed.get("kind") != "independent_multihead_recovery_paired_sealed_manifest":
        raise ValueError(f"Unexpected independent multihead sealed kind: {sealed.get('kind')}")
    if sealed.get("status") != "frozen_awaiting_truth":
        raise ValueError("Independent multihead seal is not awaiting truth")
    if sealed.get("truth_accessed") is not False or sealed.get("truth_used") is not False:
        raise ValueError("Independent multihead seal is not truth-blind")
    if sealed.get("target") != expected_target:
        raise ValueError(f"Unexpected independent multihead target: {sealed.get('target')}")
    ...
    if freeze_path != frozen_dir / "freeze.json":
        raise ValueError(f"Unexpected paired multihead freeze path: {freeze_path}")
    freeze_sha256 = freezer._sha256(freeze_path)
    if freeze_sha256 != str(sealed["freeze"]["sha256"]):
        raise ValueError("Independent multihead freeze manifest hash drift")
    freeze = heldout._read_json(freeze_path)
    if freeze.get("kind") != "independent_multihead_recovery_paired_freeze":
        raise ValueError(f"Unexpected independent multihead freeze kind: {freeze.get('kind')}")
    if freeze.get("status") != "frozen_awaiting_truth":
        raise ValueError("Independent multihead freeze is not awaiting truth")
    if freeze.get("truth_accessed") is not False or freeze.get("truth_used") is not False:
        raise ValueError("Independent multihead freeze is not truth-blind")
    if freeze.get("target") != expected_target:
        raise ValueError("Independent multihead frozen target mismatch")

    # The runner owns the paired-freeze format and verifies every role pin.
    # Keep this call before any truth path opens.
    multihead_runner.verify_paired_freeze(frozen_dir)
    pins = freeze.get("pins")
    if not isinstance(pins, Mapping):
        raise ValueError("Independent multihead paired freeze has no role-based pins")
    ...
    if str(requests_pin["snapshot_sha256"]) != str(requests_record["sha256"]):
        raise ValueError("Independent multihead prepared request pin hash mismatch")
    ...
    if actual_row_hashes != expected_row_hashes:
        raise ValueError("Independent multihead request row hash drift")
    ...
    if set(requests_by_crop) != set(paired_rows_by_crop):
        raise ValueError("Independent multihead request/prediction crop identities differ")
    ...
    if (
        ...
        raise ValueError("Independent multihead additive-invariance pin is not passed/truth-blind")
    _verify_role_hash_binding(
        pins,
        "baseline_inference",
...
def _validate_prepared_snapshot(
    ...
    if prepared.get("kind") != GATE_SPEC.prepare_kind:
        raise ValueError("Independent multihead pinned prepared-manifest kind mismatch")
    if prepared.get("status") != "prepared_awaiting_model_predictions":
        raise ValueError("Independent multihead pinned preparation is not awaiting predictions")
    if prepared.get("split") != freezer.SPLIT_NAME or prepared.get("truth_accessed") is not False:
        raise ValueError("Independent multihead pinned preparation is not truth-blind")
    if prepared.get("target") != expected_target:
        raise ValueError("Independent multihead pinned prepared target mismatch")
    config = prepared.get("independent_multihead_recovery_gate")
    if not isinstance(config, Mapping):
        raise ValueError("Independent multihead pinned preparation has no recovery contract")
    if config.get("truth_accessed") is not False or config.get("truth_used") is not False:
        raise ValueError("Independent multihead pinned recovery contract used truth")
...
def _only_pin(pins: Mapping[str, Any], role: str) -> Mapping[str, Any]:
    ...
    if not isinstance(records, list) or len(records) != 1 or not isinstance(records[0], Mapping):
        raise ValueError(f"Independent multihead freeze requires one {role} pin")
    return records[0]
...
def _find_pin(
    ...
    *,
    ...
    if not isinstance(records, list):
        raise ValueError(f"Independent multihead freeze has no {role} role")
    matches = [
        pin
        for pin in records
    ...
    if len(matches) != 1:
        raise ValueError(
            f"Independent multihead freeze cannot uniquely locate {role}/{filename} at pinned hash"
        )
    return matches[0]
...
def _find_pin_by_source(
    ...
    if len(matches) != 1:
        raise ValueError(f"Independent multihead freeze cannot locate pinned source: {expected}")
    return matches[0]
...
def _validate_row_targets(rows: Sequence[Mapping[str, Any]], *, label: str) -> None:
    for row in rows:
        ...
        if not isinstance(identity, Mapping):
            raise ValueError(f"Independent multihead {label} has no identity")
        if (
            identity.get("slug") != TARGET_SLUG
            or int(identity.get("system_index", -1)) != TARGET_SYSTEM_INDEX
        ):
            raise ValueError(f"Independent multihead {label} target identity mismatch")
        if row.get("truth_accessed") is not False:
            raise ValueError(f"Independent multihead {label} is not truth-blind")


def _validate_paired_prediction_contract(rows: Sequence[Mapping[str, Any]]) -> None:
    for row in rows:
        if row.get("truth_used") is not False:
            raise ValueError("Independent multihead paired prediction used truth")
        lanes = row.get("lanes")
        if not isinstance(lanes, Mapping) or set(lanes) != set(LANES):
            raise ValueError(
                f"Independent multihead predictions require exactly these lanes: {LANES}"
            )
        ...
        if baseline_lane.get(
            "canonical_prediction_sha256"
        ) != multihead_runner.inference._hash_json(canonical):
...
def _lane_notes(lane_payload: Any, *, lane: str) -> list[Mapping[str, Any]]:
    if not isinstance(lane_payload, Mapping):
        raise ValueError(f"Independent multihead {lane} lane is not an object")
    available = [
        name for name in ("notes", "pitch_lane") if isinstance(lane_payload.get(name), list)
    ]
    if len(available) != 1:
        raise ValueError(f"Independent multihead {lane} lane requires exactly one frozen note list")
    notes = lane_payload[available[0]]
    if any(not isinstance(note, Mapping) for note in notes):
        raise ValueError(f"Independent multihead {lane} lane contains a non-object note")
    return notes
...
def _candidate_lane(lane_payload: Any, *, lane: str) -> list[Mapping[str, Any]]:
    if not isinstance(lane_payload, Mapping) or not isinstance(
        ...
        raise ValueError(f"Independent multihead {lane} lane has no candidate_lane list")
    candidates = lane_payload["candidate_lane"]
    if any(not isinstance(candidate, Mapping) for candidate in candidates):
        raise ValueError(f"Independent multihead {lane} candidate lane contains a non-object")
    return candidates
...
def _notes_by_candidate_id(
    ...
    for note in notes:
        ...
        if not candidate_id:
            raise ValueError(f"Independent multihead {lane} note has no candidate_id")
...
def _build_report(
    *,
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "kind": "independent_multihead_recovery_one_shot_evaluation",
        "status": "evaluated_exactly_once_after_frozen_predictions",
        "target": dict(target),
        "mapping_mode": mapping_mode,
